...
if(!$sidx) $sidx =1; 
   ...
   if($searchOn=='true' or $search=='true') 
	...
	$where="where 1=1";		
	$where.=" and payment_date>$date_from and payment_date<$date_to ";
	if ($req_status!='All')
...
$result = mysql_query("SELECT COUNT(*) as count FROM payment_request $where"); 
...
{ 
 ...
 if ($page > $total_pages) 
  ...
  if($start <0)
   ...
   $SQL = "SELECT  * from payment_request $where ORDER BY $sidx $sord LIMIT $start , $limit";
	 ...
	 {	 
	   ...
	   echo "<total>".$total_pages."</total>";
		...
		echo "<cell>". $row['login_id']."</cell>";
		 echo "<cell>". $row['payment_receive_pin']."</cell>"; 
		 
		 echo "<cell>". $row['amount']."</cell>"; 
		 echo "<cell>". $row['payment_agent']."</cell>"; 
		 echo "<cell>". $row['payment_date']."</cell>"; 
