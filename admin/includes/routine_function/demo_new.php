...
if($row_routine_info=mysql_fetch_array($res_routine_info))
{
...
if($teacher_id!=0)
{
	...
	$sql_teach_ids_notin_prd = "select `teacher_id` from `std_class_routine_new` where `day`=$day and `period_id`=$period_id and `branch_id`=$branch_id and `teacher_id`!=0 $teacher_cndtn";
	//echo "<br />";
...
if($subject_id!=0)
{
	$sub_cndtn = " and `subject_id`!=".$subject_id;
}
	//$sql_sub_ids_notin_prd = "select `subject_id` from `std_class_routine_new` where `day`=$day and `period_id`=$period_id and `subject_id` not in ($subject_id,0) and `class_id`=$class_id and `branch_id`=$branch_id";
	$sql_sub_ids_notin_prd = "select `subject_id` from `std_class_routine_new` where `branch_id`=$branch_id and `subject_id`!=0 and `day`=$day and `class_id`=$class_id and `section_id`=$section_id $sub_cndtn";
	...
	<form method="POST"  action="">
    	<table style="border:0px;" cellpadding="10" cellspacing="5">
            ...
            <tr>
				...
				//echo "<br />";
				?>
                <select name="teacher_list" id="teacher_id">
            ...
            <tr>
            	...
            	<td>
                <?php
                $sql_sub = "select `id`,`subject_name`, `subject_code` from `std_subject_config` where `branch_id`='$branch_id' and `class_id`=$class_id and `id` not in ($sub_ids_notin_prd)";
				?>
