...
<?php   	include("../db.php");
			...
			move_uploaded_file($_FILES["excel_file"]["tmp_name"],"../../exam/includes/".$file_name );						
		
			require_once 'Excel/reader.php';				
			$data = new Spreadsheet_Excel_Reader();
			$data->read("../../exam/includes/".$file_name);									
            ...
            function num_convert($sb_user,$ob_user,$ct_user,$pt_user,$class_id) 
			{
                    //$query = "select SB_total,OB_total,CT_total from `tbl_class_exam_mark_distribution` where class_id=$class_id";  
					
					$query = "SELECT sub_mark,ct_mark,ob_mark,practical_mark,st_mark FROM std_subject_config
					...
					{					
				     ...
				     $total_db = $sb_db+$ob_db+$ct_db+$pt_db;    																
					 $total_user = $sb_user+$ob_user+$ct_user+$pt_user;					
			...
			function myfunc($period,$class_id,$section_id,$sub_code ,$roll,$name,$curent_row,$st,$tot,$branch_id,$file_name) 
			{
                        ...
                        $pt_mark     = $data->sheets[0]['cells'][$curent_row][$pt];						
						
						$grand_total_mark =	num_convert($sub_mark,$ob_mark,$ct_mark,$pt_mark,$class_id);																							
						...
						if( $sub_code !="" &&  $sub_mark!="" && $ob_mark !="" && $ct_mark !="" && $pt_mark!="") 	
						{								
								$query =  "insert into std_class_exam_mark_setting(class_id,class_sec,stu_id,sub_id,subjective_mark,objective_mark,ct_mark,pt_mark,total_mark,grade,point,year,period,branch_id)  
								values($class_id,'$section_id','$roll','$sub_code',$sub_mark,$ob_mark,$ct_mark,$pt_mark,$total_mark,'$gt_grade',$gt_point,$year,$period,$branch_id,$grand_total_mark)";
								mysql_query($query);
						}		   
			}
			...
			{
				if($data->sheets[0]['cells'][5][$j+3]  != "" ) 
				{
					$subject_code[] = $data->sheets[0]['cells'][5][$j+3];
				}
			}
			...
			for( $i = 1;  $i < $num_rows ; $i++ ) 
			{
				 ...
				 if($row <= $num_rows)	 
				 {				 
					$roll     = $data->sheets[0]['cells'][$i+6][1];
					$name = $data->sheets[0]['cells'][$i+6][2];						 					
