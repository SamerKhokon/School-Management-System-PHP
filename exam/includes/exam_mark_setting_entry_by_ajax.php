<?php session_start();  include("../db.php");
			...
			$pt_mark              = trim($_POST['pt_mark']);			
			$year 				   = trim($_POST["year_id"]);
			
			function get_db_total_mark($class_id,$branchid,$subject_id)
			{
				$total_db_mark_str = "SELECT ob_mark,sub_mark,ct_mark,practical_mark,final_mark FROM `std_subject_config`
				WHERE class_id=$class_id and branch_id=$branchid and subject_code=$subject_id";
				...
				$pt_db = $total_db_mark_res[3];
				return $ob_db + $sb_db +$ct_db+$pt_db;
			}
			
			$get_total = $subjective_mark+$objective_mark+$ct_mark+$pt_mark;
			$db_total  = get_db_total_mark($class_id,$branchid,$subject_id);
			...
			if($rs[0] == 0 || $rs[0]=="")
			{					
			 $insert_query = "INSERT INTO `school`.`std_class_exam_mark_setting` (`class_id`, `class_sec`, `stu_id`, `sub_id`, `subjective_mark`,`objective_mark`, `ct_mark`,`pt_mark`,`total_mark`,`grade`,`point`, `year`, `period`, `branch_id`)	VALUES($class_id, '$section_id', '$roll_id', $subject_id, $subjective_mark, $objective_mark, $ct_mark, $pt_mark, $final_result, '$ur_grade', $point, '$year', $term_id, $branchid)";
