<?php	include('db.php');		
		...
		$pre_address	     = trim($_POST['pre_address']);
		...
		$year   			 = date('Y');	
				...
				$ovr_roll = overall_class_roll($class_name);
		...
		$recent_stu_id = $year.$class_name.$section_id.$stuid_cnt;
		...
		'$emer_name1','$emer_mobile1','$emer_name2','$emer_mobile2','$email','$sex',$age,'$adm_date','$dob','$className','$group_name',
		...
		$insrt_sql = "INSERT INTO `school`.`std_profile`(`stu_id`,`name`,`father_name`,`mother_name`,`pre_address`,`par_address`, 
		...
		`priv_school_class_roll`,`priv_school_remarks`, `priv_school_result`,`present_class`, `present_class_roll`, `st_status`,`branch_id`,`ovr_roll`)	
		VALUES('$recent_stu_id','$stu_name','$father_name','$mother_name','$pre_address','$par_address', '$h_tel','$emer_name1','$emer_mobile1','$emer_name2','$emer_mobile2','$email','$sex','$age','$adm_date','$dob','$class_name','$max_classroll', '$group_name','$section_name','$father_income','$mother_income','$father_rank','$mother_rank','custom','custom', 
		'$father_wrk_phone','$mother_wrk_phone','custom','$pre_school_name','$pre_school_add', '$pre_class','$pre_class_roll','custom','$pre_class_result','$className','custom','available',$branchid,$ovr_roll);";		
		...
		`stuts`,`year`,`branch_id`,`ovr_roll`)
		VALUES($class_name,$section_id,'$className','$section_name','$recent_stu_id','$stu_name',$max_classroll,'Admitted','$year',$branchid,$ovr_roll)";
        mysql_query($std_student_info_str);
		
		
		//data insert into std_class_info table
		$class_info_add = "INSERT INTO `school`.`std_class_info`
		(`class_id`,`class_name`,`class_sec`,`stu_id`,`stu_name`,`class_roll`,`year`,`branch_id`,`ovr_roll`) 
		VALUES($class_name,'$className','$section_name',$recent_stu_id,'$stu_name',$max_classroll,'$year',$branchid,$ovr_roll)";	
		mysql_query($class_info_add);	
		
		echo 'Data submitted successfully!';
		
	    function overall_class_roll($class_name) {
		  $str = "select count(*) from std_class_info where class_id=$class_name";
		  $sql = mysql_query($str);
